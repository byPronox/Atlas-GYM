...
- Crea la clase de la acción en el paquete `org.example.AtlasGym.acciones`, normalmente extendiendo `ViewBaseAction`
...
<controlador nombre="TuPrimeraEntidad">
    ...
    <accion nombre="mostrarImporte" modo="detail"
        clase="org.example.AtlasGym.acciones.MostrarImporte"/>
</controlador>
```

```java
package org.example.AtlasGym.acciones;
...
- ✅ Coloca las clases de acción en `org.example.AtlasGym.acciones`
...
```java
package org.example.AtlasGym.pruebas;
...
- Crea un paquete `cuadrosmando` (ej. `org.example.AtlasGym.cuadrosmando`) y coloca la clase `CuadroMando` y sus clases auxiliares en él.
